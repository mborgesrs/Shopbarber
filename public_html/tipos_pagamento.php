...
$return_url = $_GET['return_url'] ?? 'tipos_pagamento.php';

if($_SERVER['REQUEST_METHOD']==='POST'){
    if(isset($_POST['delete_id'])){
        $stmt = $pdo->prepare('DELETE FROM tipos_pagamento WHERE id = ? AND company_id = ?');
        $stmt->execute([$_POST['delete_id'], $company_id]);
    } elseif (isset($_POST['id']) && !empty($_POST['id'])) {
        $stmt = $pdo->prepare('UPDATE tipos_pagamento SET descricao = ? WHERE id = ? AND company_id = ?');
        $stmt->execute([$_POST['name'], $_POST['id'], $company_id]);
    } else {
        $stmt = $pdo->prepare('INSERT INTO tipos_pagamento (descricao, company_id) VALUES (?, ?)');
        $stmt->execute([$_POST['name'], $company_id]);
    }
    header('Location: ' . $return_url); exit;
}

$stmt = $pdo->prepare('SELECT * FROM tipos_pagamento WHERE company_id = ? ORDER BY descricao');
...
<div class="flex items-center justify-between mb-6">
    <div class="flex items-center gap-4">
        ...
        <h2 class="text-2xl font-bold text-gray-800">Tipos de Pagamento</h2>
    </div>
            ...
            <tr class="hover:bg-gray-50 transition-colors group">
                ...
                <td class="px-6 py-2 text-right">
                    <div class="flex justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onclick='openModal(<?= json_encode(["id" => $item["id"], "name" => $item["descricao"]]) ?>)' class="w-7 h-7 rounded-lg text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-all">
                            <i class="fas fa-edit text-xs"></i>
                        </button>
                        <form method="post" class="inline" onsubmit="confirmFormSubmit(event, this, 'Deseja realmente excluir este tipo de pagamento?')">
            ...
            <?php if(!$list): ?>
                <tr>
                    <td colspan="2" class="px-6 py-12 text-center">
                        <div class="text-gray-300 mb-2"><i class="fas fa-money-bill-wave text-4xl"></i></div>
                        <p class="text-gray-400">Nenhum tipo de pagamento encontrado.</p>
                    </td>
                </tr>
            <?php endif; ?>
        </tbody>
...
<div id="modal-form" class="hidden fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md animate-in fade-in zoom-in duration-200">
        <div class="p-6 border-b border-gray-100 flex justify-between items-center">
            <h3 id="modal-title" class="font-bold text-lg text-gray-800">Novo Tipo de Pagamento</h3>
            <button onclick="closeModal()" class="text-gray-400 hover:text-gray-600 text-xl font-light">×</button>
        </div>
        <form method="post" action="tipos_pagamento.php?return_url=<?= urlencode($return_url) ?>" class="p-6">
            <input type="hidden" name="id" id="field-id">
            <div class="mb-6">
                <label class="block text-sm font-bold text-gray-700 mb-2">Descrição</label>
                <input type="text" name="name" id="field-name" required class="w-full border border-gray-200 p-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 placeholder-gray-300" placeholder="Ex: Cartão de Crédito 10x">
            </div>
            <div class="flex gap-3">
                ...
                <button class="flex-1 bg-blue-600 text-white px-4 py-2.5 rounded-xl font-bold text-sm hover:bg-blue-700 transition-all shadow-lg shadow-blue-100">Salvar Tipo</button>
            </div>
        </form>
    </div>
</div>
...
function openModal(data = null) {
    ...
    if (data) {
        title.innerText = 'Editar Tipo de Pagamento';
        fieldId.value = data.id;
        fieldName.value = data.name;
    } else {
        title.innerText = 'Novo Tipo de Pagamento';
        fieldId.value = '';
        fieldName.value = '';
    }
